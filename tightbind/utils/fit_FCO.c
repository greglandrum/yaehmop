...
#define READ_FROM_LOOKUP_TBL(__tbl__,__x__,__val__)\
{\
  real loc_var =(__x__)-__tbl__.min_val;\
  __val__=( (__x__) < __tbl__.max_val && loc_var > 0 ) ? \
 __tbl__.values[(int)floor(loc_var*__tbl__.inv_step + 0.5)] : 0.0;\
}
...
#endif
{
  ...
  real *frag_vals,temp_diff;
  real total_DOS,tempval;
  real norm_val,gauss_val;
  real tp_frag_E,tp_total_E;
  real lookup_max,lookup_inv_step;
  real *lookup_values;
...
//  argc = ccommand(&argv);
  ...
  ********/
  ...
  lookup_max = gauss_lookup_table.max_val;
  lookup_inv_step = gauss_lookup_table.inv_step;
  lookup_values = gauss_lookup_table.values;
  ...
  ********/
  ...
  for(i=0;i<FCO_points_so_far;i++){
    ...
    tp_frag_E = this_point->frag_E;
    tp_total_E = this_point->total_E;
    norm_val = this_point->value * norm_fact;
    ...
    for(k=0;k<points_per_side;k++,curr_tot_E += E_step){
      ...
      if( tot_E_diff < E_CUTOFF ){
        ...
        for(j=0;j<points_per_side;j++,curr_frag_E += E_step){
          E_diff = tot_E_diff + fabs(tp_frag_E - curr_frag_E);

          if( E_diff <= E_CUTOFF ){
            tempval = broadening*E_diff*E_diff;
            if (tempval < lookup_max && tempval > 0 ){
              results[ktab + j] += norm_val *
                lookup_values[(int)floor(tempval*lookup_inv_step+ 0.5)];
            }
calls_to_exp++;
          }
        }
