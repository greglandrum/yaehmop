...
;/hdr/ ****************************************************************************************
;
; Copyright 2002-15 Great Migrations LLC ALL RIGHTS RESERVED
;
; The <b>fortran.h</b> header file is the FORTRAN system library header file. Whenever any
; methods in the Fortran compiler runtime library are used this header MUST be included. If
; the output of pFortran is being processed, then one of the following symbols must must be
; defined: SPROTOTYPE, LPROTOTYPE, SKANDRCCOM, LKANDRCCOM,I860COMPIL,VMSCCOMPIL or HXLCCOMPIL.
;
;/hdr/ ****************************************************************************************
*/
...
*/
#ifdef FPROTOTYPE
longlong fifi8max0(longlong a1,longlong a2);
longlong fifi8min0(longlong a1,longlong a2);
longlong fifi8mod(longlong num,longlong dem);
#endif
/*
...
int fiordb(USHORT* bVal, int nval);
...
int fiordt(ULONG* bVal, int nval);
...
int fiowrb(USHORT* bVal, int nval);
...
int fiowrt(ULONG* bVal, int nval);
...
Dope* MakeDope(int Lenth);
extern int FtnNjp;
extern jmp_buf FtnJmp[];
#define Cgo_init(x) *(x)=++FtnNjp,setjmp(FtnJmp[FtnNjp])

#define perform(x,y) BlkReturns[x]=y; goto PROC_##x; PERFORM##y:
#define FinishProc(x) Nsub=BlkReturns[x];goto GO_SUB;
#endif
